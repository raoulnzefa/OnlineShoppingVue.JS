<template>
    <div class="container app">
        <!-- 1-image upload
        2- minimize code , change branch to select if main changed
         -->
        <vue-form :state='formState' @submit.prevent="onSubmit" > 
            ...
            <button class=" cta"  type="submit" :disabled='!this.formState.$dirty'>
                 <span>Save Data</span>
...
export default {
    ...
    data(){
        return{
            ...
            model:{ MainCategory:{_id:'',Name:''} },SaleVal:[{name:'ON',value:true},{name:'OFF',value:false}]
        }
    },
      created:function(){// to hide main Header
        this.navShow();
        Vue.axios.get(this.CategoryURL).then(data=>this.categories=data.data).catch(()=>{});
        Vue.axios.get(this.OneProductURL+'/'+this._id).then(data=>{
            this.model=data.data;
            this.model.MainCategory={_id:this.model.Category._id,name:this.model.Category.MainCategory};
            this.model.BranchCategory=this.model.Category._id;
            this.ImageURL = this.model.Image;
            $('#Image').val(this.ImageURL);
            }).catch(()=>{});
    },
    methods:{
        ...
        onSubmit:function(){
            ...
            if(this.formState.$valid&&!this.ValidateImage){// for save  Data 
                Vue.axios.put(this.ProductsURL,this.formData,{headers: {
